...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // POST & PUT /api/properties — create or update property in DB
    if (req.method === 'POST' || req.method === 'PUT') {
      const listing = typeof req.body === 'string' ? JSON.parse(req.body) : (req.body || {});
      if (!listing || !listing.id || !listing.title) {
        return res.status(400).json({ success: false, error: 'Invalid listing payload: missing id or title.' });
      }

      const ownerId = listing.ownerId || 'admin';
      const personalInfoJson = JSON.stringify(listing.personalOwnerInfo || {});

      // 1. Try updating existing listing
      ...
      if (updateResult && updateResult.length > 0) {
        return res.status(200).json({ success: true, listing, isLiveDb: true, action: 'updated' });
      }

      // 2. If no record was updated, insert new listing
      await sql`
        INSERT INTO listings (
          ...
          ${listing.squareMeters}, ${listing.amenities || []}, ${listing.status || 'approved'}, 
          ${ownerId}, ${listing.approvedByAdminId || null}, ${listing.image || ''}, 
          ${listing.images || []},
          ${personalInfoJson}::jsonb, ${listing.createdAt || new Date().toISOString()}
        );
      `;

      return res.status(200).json({ success: true, listing, isLiveDb: true, action: 'inserted' });
    }
