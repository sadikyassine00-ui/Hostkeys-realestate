...
export default function DashboardView({
  ...
  // Active workspace subsection state: 'pending' or 'approved' or 'profile'
  ...
  const totalSystemBrokeredVolume = convertValue(baseBrokeredVolume, currency, eurRate);
  ...
  const myPortfolioValue = convertValue(basePortfolioValue, currency, eurRate);
  ...
  return (
    <div id="dashboard-view-container" className="space-y-8 animate-fadeIn">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              />
              <span className="absolute -bottom-1 -right-1 h-5 w-5 bg-brand text-[#030303] rounded-full flex items-center justify-center text-[10px] font-bold">
                ✓
              </span>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono tracking-wide uppercase bg-brand/10 text-brand border border-brand/20">
                  {isAdmin ? (lang === 'fr' ? '🛡️ Administrateur Hostkeys' : '🛡️ Hostkeys Admin') : (lang === 'fr' ? '👤 Propriétaire' : '👤 Property Owner')}
                </span>
              </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-brand text-[#030303] hover:bg-brand/90 font-bold font-mono text-xs flex items-center gap-2 transition-all shadow-[0_0_15px_rgba(166,254,0,0.2)] cursor-pointer"
            >
        ...
        {isAdmin ? (
          <>
            <div className="bg-[#0c0c0c] border border-neutral-850 p-5 rounded-2xl">
              ...
              <div className="text-2xl font-bold font-mono text-brand mt-1">
                {formatCurrency(baseBrokeredVolume, currency, eurRate)}
              </div>
            ...
            <div className="bg-[#0c0c0c] border border-neutral-850 p-5 rounded-2xl">
              <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">{lang === 'fr' ? 'Audits en Attente' : 'Pending Verification Queue'}</span>
            ...
            <div className="bg-[#0c0c0c] border border-neutral-850 p-5 rounded-2xl">
              <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">{lang === 'fr' ? 'Propriétés Actives' : 'Live Approved Properties'}</span>
          ...
          <>
            <div className="bg-[#0c0c0c] border border-neutral-850 p-5 rounded-2xl">
              ...
              <div className="text-2xl font-bold font-mono text-brand mt-1">
                {formatCurrency(basePortfolioValue, currency, eurRate)}
              </div>
      ...
      {/* NAVIGATION TABS */}
      <div className="flex border-b border-neutral-850 gap-4 text-xs font-mono">
        {isAdmin ? (
          <>
            <button
              onClick={() => setAdminTab('pending')}
              className={`pb-3 font-semibold transition-all border-b-2 ${adminTab === 'pending' ? 'border-brand text-brand' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              {lang === 'fr' ? '📋 Biens en Attente d\'Audit' : '📋 Pending Verification Queue'} ({totalPendingListings.length})
            </button>
            <button
              onClick={() => setAdminTab('approved')}
              className={`pb-3 font-semibold transition-all border-b-2 ${adminTab === 'approved' ? 'border-brand text-brand' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              {lang === 'fr' ? '✅ Propriétés Actives' : '✅ Live Properties'} ({totalApprovedListings.length})
            </button>
            <button
              onClick={() => setAdminTab('profile')}
              className={`pb-3 font-semibold transition-all border-b-2 ${adminTab === 'profile' ? 'border-brand text-brand' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              {lang === 'fr' ? '⚙️ Mon Profil' : '⚙️ My Profile Settings'}
            </button>
          </>
        ) : (
          <>
            <button
              ...
              className={`pb-3 font-semibold transition-all border-b-2 ${ownerTab === 'listings' ? 'border-brand text-brand' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              {lang === 'fr' ? '🏠 Mes Biens Immobiliers' : '🏠 My Properties'} ({mySubmissions.length})
            </button>
            <button
              onClick={() => setOwnerTab('profile')}
              className={`pb-3 font-semibold transition-all border-b-2 ${ownerTab === 'profile' ? 'border-brand text-brand' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              {lang === 'fr' ? '⚙️ Mon Profil' : '⚙️ My Profile Settings'}
            </button>
          </>
        ...
        {isAdmin ? (
          adminTab === 'pending' ? (
            <div className="space-y-4">
              ...
              {totalPendingListings.length === 0 ? (
                <div className="text-center py-16 bg-[#030303] rounded-xl border border-neutral-900">
                  ...
                  <p className="text-xs font-mono text-slate-400 mt-1">{lang === 'fr' ? 'Toutes les soumissions ont été vérifiées et publiées.' : 'All submitted properties have been reviewed.'}</p>
                </div>
              ) : (
                <div className="space-y-4">
                    ...
                    <div key={listing.id} className="bg-[#030303] border border-neutral-850 p-4 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-mono text-brand mt-0.5">{formatCurrency(listing.price, currency, eurRate, true, listing.type)} • {listing.location}</p>
                          <p className="text-[11px] font-mono text-slate-400 mt-1">Owner: {listing.personalOwnerInfo?.name || listing.ownerId} ({listing.personalOwnerInfo?.email || 'N/A'})</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 self-end md:self-center">
                        <button
                          ...
                          className="px-4 py-2 rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500 hover:text-black font-mono text-xs font-bold transition-all border border-emerald-500/20"
                        >
                          ✓ {lang === 'fr' ? 'Approuver' : 'Approve'}
                        </button>
                        <button
                          onClick={() => onReject(listing.id)}
                          className="px-4 py-2 rounded-lg bg-rose-500/10 text-rose-400 hover:bg-rose-500 hover:text-white font-mono text-xs font-bold transition-all border border-rose-500/20"
                        >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {totalApprovedListings.map(listing => (
                  <div key={listing.id} onClick={() => onSelectListing(listing)} className="bg-[#030303] border border-neutral-850 rounded-xl p-3 cursor-pointer hover:border-brand/30 transition-all">
                    ...
                    <p className="text-xs font-mono text-brand mt-1">{formatCurrency(listing.price, currency, eurRate, true, listing.type)}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <form onSubmit={handleProfileSave} className="max-w-md space-y-4 font-mono text-xs">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-xl bg-brand text-[#030303] font-bold hover:bg-brand/90 transition-all">
          ...
          ownerTab === 'listings' ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-neutral-900 pb-3">
                ...
                <button onClick={onAddListing} className="px-3 py-1.5 rounded-lg bg-brand text-[#030303] font-bold font-mono text-xs flex items-center gap-1">
              ...
              {mySubmissions.length === 0 ? (
                <div className="text-center py-16 bg-[#030303] rounded-xl border border-neutral-900">
                  ...
                  <p className="text-xs font-mono text-slate-400 mt-1 max-w-sm mx-auto">{lang === 'fr' ? 'Vous n\'avez pas encore publié de propriété. Cliquez sur le bouton pour soumettre votre premier bien.' : 'You have not listed any properties yet. Click "+ Add Property" to publish your first listing.'}</p>
                  <button onClick={onAddListing} className="mt-4 px-4 py-2 rounded-xl bg-brand text-[#030303] font-bold font-mono text-xs inline-flex items-center gap-2">
                    <Plus className="h-4 w-4" />
