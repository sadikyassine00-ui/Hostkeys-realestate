...
export default function PropertyForm({ currentUser, onAddListing, onClose, currency, eurRate, lang }: PropertyFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (imageType === 'url' && customImageUrl.trim()) {
      finalImageUrl = customImageUrl.trim();
    } else if (imageType === 'upload' && uploadedImage) {
    ...
    onAddListing({
      ...
      type,
      price: Number(price),
      location: finalLocation || 'Marrakech',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <div className="bg-[#0c0c0c] border border-neutral-850 rounded-2xl w-full max-w-2xl p-6 md:p-8 space-y-6 shadow-2xl my-8">
        <div className="flex items-center justify-between border-b border-neutral-900 pb-4">
          ...
          <button onClick={onClose} className="p-1.5 rounded-full hover:bg-neutral-900 text-slate-400 hover:text-white">
            ✕
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 bg-[#030303] p-1 rounded-xl border border-neutral-850">
              <button
                type="button"
                onClick={() => setType('buy')}
                className={`py-2 rounded-lg font-bold text-center transition-all ${type === 'buy' ? 'bg-brand text-[#030303]' : 'text-slate-400'}`}
              >
              ...
              <button
                ...
                className={`py-2 rounded-lg font-bold text-center transition-all ${type === 'rent' ? 'bg-brand text-[#030303]' : 'text-slate-400'}`}
              >
          ...
          <div>
            ...
            <input
              ...
              placeholder={lang === 'fr' ? 'ex: Villa Moderne avec Piscine à Marrakech' : 'e.g., Luxury Villa with Private Pool'}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={lang === 'fr' ? 'Décrivez les atouts, équipements et pièces...' : 'Describe the architectural layout, amenities, and unique features...'}
              className="w-full bg-[#030303] border border-neutral-800 rounded-xl px-3.5 py-2.5 text-white focus:border-brand focus:outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              />
              {price !== '' && (
                <span className="text-[10px] text-brand mt-1 block">
                  ≈ {formatCurrency(Number(price), currency, eurRate)}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 bg-[#030303] p-3 rounded-xl border border-neutral-850 max-h-36 overflow-y-auto">
              {ALL_AMENITIES.map(amenity => (
                <button
                  ...
                  className={`p-2 rounded-lg text-[10px] text-left transition-all border ${selectedAmenities.includes(amenity) ? 'bg-brand/10 border-brand text-brand font-bold' : 'border-neutral-850 text-slate-400 hover:text-white'}`}
                >
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-neutral-900">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 rounded-xl border border-neutral-800 text-slate-400 hover:text-white"
            >
              {lang === 'fr' ? 'Annuler' : 'Cancel'}
            </button>
            <button
              type="submit"
              className="px-6 py-2.5 rounded-xl bg-brand text-[#030303] font-bold hover:bg-brand/90 transition-all shadow-[0_0_15px_rgba(166,254,0,0.2)]"
            >
